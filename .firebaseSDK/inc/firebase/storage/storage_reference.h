...
class StorageReference {
 public:
  ...
  /// @brief List up to `maxResults` items (files) and prefixes (folders) under
  /// this StorageReference.
  /// "/" is treated as a path delimiter. Firebase Storage does not support
  /// unsupported object paths that end with "/" or contain two consecutive
  /// "/"s. All invalid objects in GCS will be filtered.
  /// `list(maxResults)` is only available for projects using Firebase Rules
  /// Version 2.
  ///
  /// @param[in] maxResults The maximum number of results to return in a single
  /// page. Must be greater than 0 and at most 1000.
  /// @param[in] pageToken A page token from a previous call to list.
  ///
  /// @returns A future that returns the ListResult.
  Future<ListResult> List(int maxResults, std::string pageToken);
